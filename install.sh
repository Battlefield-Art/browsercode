...
if [ -n "$binary_path" ]; then
    ...
    if [ -z "$requested_version" ]; then
        ...
        if [[ $? -ne 0 || -z "$specific_version" ]]; then
            ...
            echo -e "${MUTED}If the repo is private, pin a version: --version <ver>${NC}"
            exit 1
        fi
    else
...
# `uv` is required at runtime for the `browser_execute` tool (the harness uses
# it to manage its Python venv). Detect and warn if missing — never auto-install.
